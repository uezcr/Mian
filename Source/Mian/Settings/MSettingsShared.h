...
#include "MSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EMAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UMSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UMSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UMSettingsShared, FOnSettingChangedEvent, UMSettingsShared* Settings);
...
public:
	...
	static UMSettingsShared* CreateTemporarySettings(const UMLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UMSettingsShared* LoadOrCreateSettings(const UMLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UMSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UMLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EMAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EMAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EMAllowBackgroundAudioSetting AllowAudioInBackground = EMAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EMGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EMGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EMGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EMGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EMGamepadSensitivity GamepadLookSensitivityPreset = EMGamepadSensitivity::Normal;
	UPROPERTY()
	EMGamepadSensitivity GamepadTargetingSensitivityPreset = EMGamepadSensitivity::Normal;
