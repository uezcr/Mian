#include "MInputModifiers.h"

#include "EnhancedPlayerInput.h"
#include "GameFramework/PlayerController.h"
#include "Input/MAimSensitivityData.h"
#include "Player/MLocalPlayer.h"
#include "Settings/MSettingsShared.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(MInputModifiers)

DEFINE_LOG_CATEGORY_STATIC(LogMInputModifiers, Log, All);

//////////////////////////////////////////////////////////////////////
// MInputModifiersHelpers

namespace MInputModifiersHelpers
{
	/** Returns the owning MLocalPlayer of an Enhanced Player Input pointer */
	static UMLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UMLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
//////////////////////////////////////////////////////////////////////
// UMSettingBasedScalar

FInputActionValue UMSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UMLocalPlayer* LocalPlayer = MInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UMSettingsShared::StaticClass();
			UMSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
//////////////////////////////////////////////////////////////////////
// UMInputModifierDeadZone

FInputActionValue UMInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UMLocalPlayer* LocalPlayer = MInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}
	
	UMSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UMInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue, FInputActionValue FinalValue) const
{
...
//////////////////////////////////////////////////////////////////////
// UMInputModifierGamepadSensitivity

FInputActionValue UMInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const EMGamepadSensitivity Sensitivity = (TargetingType == EMTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
//////////////////////////////////////////////////////////////////////
// UMInputModifierAimInversion

FInputActionValue UMInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	UMLocalPlayer* LocalPlayer = MInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
	{
