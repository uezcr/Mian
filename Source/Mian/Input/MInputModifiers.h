...
#include "MInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UMAimSensitivityData;
class UObject;
...
class UMSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the M Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "M Settings Driven Dead Zone"))
class UMInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EMTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in M Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "M Gamepad Sensitivity"))
class UMInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EMTargetingType TargetingType = EMTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UMAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the M Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "M Aim Inversion Setting"))
class UMInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
