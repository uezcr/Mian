...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EMEditorLoadMode
{
...
namespace MGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("M.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via MGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UMGameplayCueManager::DumpGameplayCues));

	static EMEditorLoadMode LoadMode = EMEditorLoadMode::LoadUpfront;
}
...
UMGameplayCueManager::UMGameplayCueManager(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
}

UMGameplayCueManager* UMGameplayCueManager::Get()
{
	return Cast<UMGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UMGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UMGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogMian, Warning, TEXT("UMGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UMGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (MGameplayCueManagerCvars::LoadMode)
	{
	case EMEditorLoadMode::LoadUpfront:
		return true;
	case EMEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EMEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UMGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UMGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UMGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
		UMGameplayCueManager* GCM = Cast<UMGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogMian, Error, TEXT("DumpGameplayCues failed. No UMGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogMian, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogMian, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogMian, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogMian, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogMian, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogMian, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogMian, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogMian, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogMian, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogMian, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogMian, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogMian, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UMGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UMGameplayCueManager* StrongThis = Get())
					{
...
void UMGameplayCueManager::HandlePostGarbageCollect()
{
...
void UMGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogMian, Warning, TEXT("UMGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UMGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (MGameplayCueManagerCvars::LoadMode)
	{
	case EMEditorLoadMode::LoadUpfront:
...
void UMGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject,
...
void UMGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UMGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UMGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UMGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UMGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
