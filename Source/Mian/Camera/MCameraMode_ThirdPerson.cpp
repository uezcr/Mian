#include "MCameraMode_ThirdPerson.h"
#include "Camera/MCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/MPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "MCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(MCameraMode_ThirdPerson)

namespace MCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UMCameraMode_ThirdPerson::UMCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;
	bUseRuntimeFloatCurves = false;
	AimLineToDesiredPosBlockedPct = 0.f;
	
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FMPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UMCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UMCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UMCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UMCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IMCameraAssistInterface* TargetControllerAssist = Cast<IMCameraAssistInterface>(TargetController);

	IMCameraAssistInterface* TargetActorAssist = Cast<IMCameraAssistInterface>(TargetActor);
	...
	IMCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IMCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IMCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IMCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UMCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IMCameraTarget.GetIgnoredActorsForCameraPent-ration();
	...
	{
		FMPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(MCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UMCameraMode_ThirdPerson::SetTargetCrouchOffset(const FVector& NewTargetOffset)
{
...
void UMCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
