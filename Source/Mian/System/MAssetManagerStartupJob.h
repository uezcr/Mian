...
DECLARE_DELEGATE_OneParam(FMAssetManagerStartupJobSubstepProgress, float /*NewProgress*/);

struct FMAssetManagerStartupJob
{
	FMAssetManagerStartupJobSubstepProgress SubstepProgressDelegate;
	TFunction<void(const FMAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)> JobFunc;
	FString JobName;
	float JobWeight;
	mutable double LastUpdate = 0;

	/** Simple job that is all synchronous */
	FMAssetManagerStartupJob(const FString& InJobName, const TFunction<void(const FMAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)>& InJobFunc, float InJobWeight)
		: JobFunc(InJobFunc)
		, JobName(InJobName)
