#include "System/MGameInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MGameInstance)

namespace M
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarMTestEncryption(
		TEXT("M.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarMUseDTLSEncryption(
		TEXT("M.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using M.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarMTestDTLSFingerprint(
		TEXT("M.TestDTLSFingerprint"),
...
AMPlayerController* UMGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AMPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UMGameInstance::CanJoinRequestedSession() const
{
...
void UMGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error,
	ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UMLocalPlayer* LocalPlayer = Cast<UMLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UMGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken,
	const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (M::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (M::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("MTest.pem");
...
void UMGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UMGameInstance::Init()
{
	...
	// {
	// 	ComponentManager->RegisterInitState(MGameplayTags::InitState_Spawned, false, FGameplayTag());
	// 	ComponentManager->RegisterInitState(MGameplayTags::InitState_DataAvailable, false, MGameplayTags::InitState_Spawned);
	// 	ComponentManager->RegisterInitState(MGameplayTags::InitState_DataInitialized, false, MGameplayTags::InitState_DataAvailable);
	// 	ComponentManager->RegisterInitState(MGameplayTags::InitState_GameplayReady, false, MGameplayTags::InitState_DataInitialized);
	// }
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UMGameInstance::OnPreClientTravelToSession);
	}
}
...
void UMGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (M::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (M::bUseDTLSEncryption)
		{
